...
ui <- fluidPage(
  ...
  titlePanel("Угадай слово или будешь повешен !!!"),
  ...
  bootstrapPage(
    ...
    mainPanel(
      div(class="wrapper",
          ...
          HTML("<canvas id='spectrum1' class='spectrum' width=800 height=400 onclick='spectrum1_click(event)'></canvas>")
      )
    )
    )
    )
    ...
    if (sum(grepl('_', dt_word$guessed)) == 0) {
      ...
      showModal(modalDialog(
        tags$div(
          ...
          actionButton(
            inputId = "reload",
            label = "Не отчаивайтесь! Попробуйте еще раз!",
            style = "width: 100%;"
          )
        ),
        footer = NULL,
